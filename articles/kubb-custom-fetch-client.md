...
プラグインがコピーする `kubb-plugins/templates/neverthrow.ts.txt` は、「[自分で整理したコード](#どういうコードを生成させるか考える)」の「共通化できる部分」の import 先を `./client` に変えたものです。

:::details templates/neverthrow.ts.txt
```ts:kubb-plugins/templates/neverthrow.ts.txt
import { err, ok, type Result, ResultAsync } from "neverthrow";
import type {
  CallResult,
  DataOf,
  DataShape,
  Options as KubbOptions,
  SuccessOf,
  SuccessStatusCode,
  ToStatusNumber,
} from "./client";
