...
- **ESLint** (`eslint-plugin-boundaries` 等) - おそらく最も普及している。`no-restricted-imports` やプラグインベース
- **Steiger** - FSD 公式リンター。専用 CLI でルールをチェックできる。LSPがサポートされていない。
- **Biome GritQL** - Biome 2.0 から使えるカスタムプラグイン。**この記事の主題** 。
